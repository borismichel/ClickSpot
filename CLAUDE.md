...
- **Frontend** (`frontend/src/components/settings/SyncTab.tsx`): last-refreshed (from `/api/v1/metadata` freshness timestamps), Sync now button (disabled without `HUBSPOT_TOKEN` or while a run is in flight), a "Keep my data up to date automatically" switch with the next-refresh time, 4-stage progress, failure alert with a deep link into the Dagster run (`DAGSTER_UI_URL`, browser-facing, separate from `DAGSTER_GRAPHQL_URL`). Polls slowly while idle so schedule changes made in the Dagster UI show up.
