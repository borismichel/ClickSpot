...
- **Per-portal silver extensions**: operators add or remove columns through Settings → Properties, which writes `extraction.silver_properties.<dim>` into `~/.clickspot/customer.json`; `silver_config._compose_columns` merges those into the core lists at import, and `silver_config.recompose_columns()` / `app.config.rebuild_tables()` (wrapped by `app/schema_refresh.py`) re-derive them at runtime so the backend never needs a restart. A save only records the change and raises a pending-apply flag (`extraction.mark_pending_apply`); the served schema refreshes when the operator's **Apply changes** rebuild lands (see Sync surface), so chat never describes columns the warehouse doesn't hold yet. The legacy `silver_config_custom.py` (gitignored, worked example at `silver_config_custom.py.example`) still appends deal/contact tuples with a deprecation warning; it cannot reach a container deployment, since released images are built from a clean checkout.
...
### Sync surface (Settings → Data sync + Apply changes)

- **Backend** (`app/api/sync_routes.py` + `app/dagster_client.py` + `app/sync_naming.py`): one-button full refresh for operators who think in HubSpot terms. `POST /api/v1/sync` launches `bronze_job` with `clickspot/sync` + `clickspot/sync_id` correlation tags; the chaining sensors (`sensors.py`) propagate `clickspot/*` tags onto each run they request, so all four runs of a sync come back from one tag-filtered GraphQL query. `GET /api/v1/sync/status` reports stage progress in operator language (`app/sync_naming.py` maps step keys → HubSpot names, generic phrase for bridge/internal tables) and degrades gracefully when Dagster is unreachable. `PUT /api/v1/sync/schedule` turns the hourly schedule on/off via Dagster's start/stop mutations; the status payload's `schedule` block reports the orchestrator's live instigator state plus the next tick time. The schedule itself (`schedules.py`) stamps the same `clickspot/*` tags on each tick's bronze run (sync id derived from the tick time), so scheduled refreshes appear in the tab exactly like button-started ones. Its Dagster name stays `bronze_job_schedule` so already-started schedule state survives upgrades.
- **Apply changes** (`POST /api/v1/sync/apply`): makes a saved settings edit live end to end — reloads the Dagster code locations, launches `silver_job` with the same correlation tags plus `clickspot/kind: apply` (sensors chain gold → anon; no HubSpot fetch, no token needed). Once the silver rebuild lands, the backend refreshes its own served schema (`app/schema_refresh.py`), and on full success clears the pending-apply flag. A watcher thread finalizes even if nobody polls; `GET /sync/status` finalizes as a fallback and reports `kind` (apply = 3 stages) + `pending_apply`. The MCP server re-derives the table catalog per schema request — gated on the pending flag, so it follows *applied* configuration — and picks up changes without a restart.
- **Frontend** (`frontend/src/components/settings/SyncTab.tsx`, `ApplyChangesBanner.tsx`, shared `SyncProgress.tsx`): last-refreshed (from `/api/v1/metadata` freshness timestamps), Sync now button (disabled without `HUBSPOT_TOKEN` or while a run is in flight), a "Keep my data up to date automatically" switch with the next-refresh time, staged progress, failure alert with a deep link into the Dagster run (`DAGSTER_UI_URL`, browser-facing, separate from `DAGSTER_GRAPHQL_URL`). Polls slowly while idle so schedule changes made in the Dagster UI show up. A save in the Extraction/Properties tabs raises the server-backed pending banner on the Settings page; its **Apply changes** button drives the apply flow through the same progress/failure surface.
...
- Adding a silver property: for a core column shipped to everyone, add one tuple `(silver_name, bronze_key, type)` to the config dict in `silver_config.py`; per-portal columns go through Settings → Properties instead (save, then **Apply changes** in the banner — it reloads the code location, rebuilds silver → gold → anon, and refreshes the served schema in one step)
...
`docs/agents/` is gitignored — it holds each developer's own answers for the engineering
skills, regenerated by `/setup-matt-pocock-skills`. A fresh clone has none of it, and the
summaries below describe this clone's setup, not a project-wide convention.

### Issue tracker

Local markdown under `.scratch/<feature-slug>/` (also gitignored); GitHub carries pull requests, not issues. See `docs/agents/issue-tracker.md`.

### Triage labels

The five canonical roles, unchanged, written as the `Status:` line in each issue file. See `docs/agents/triage-labels.md`.
