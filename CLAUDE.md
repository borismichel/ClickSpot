...
- **Apply changes** (`POST /api/v1/sync/apply`): makes a saved settings edit live end to end — reloads the Dagster code locations, launches `silver_job` with the same correlation tags plus `clickspot/kind: apply` (sensors chain gold → anon; no HubSpot fetch, no token needed). Once the silver rebuild lands, the backend refreshes its own served schema (`app/schema_refresh.py`), and on full success clears the pending-apply flag. A watcher thread finalizes even if nobody polls; `GET /sync/status` finalizes as a fallback and reports `kind` (apply = 3 stages) + `pending_apply`. The MCP server re-derives the table catalog per schema request, so applied changes reach it without a restart.
