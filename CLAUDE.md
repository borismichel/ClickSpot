...
- **Frontend** (`frontend/src/components/settings/SyncTab.tsx`, `ApplyChangesBanner.tsx`, shared `SyncProgress.tsx`): last-refreshed (from `/api/v1/metadata` freshness timestamps), Sync now button (disabled without `HUBSPOT_TOKEN` or while a run is in flight), staged progress, failure alert with a deep link into the Dagster run (`DAGSTER_UI_URL`, browser-facing, separate from `DAGSTER_GRAPHQL_URL`). A save in the Extraction/Properties tabs raises the server-backed pending banner on the Settings page; its **Apply changes** button drives the apply flow through the same progress/failure surface.
