...
| `DAGSTER_GRAPHQL_URL` | No | Dagster GraphQL endpoint the backend calls — code-location reloads after a Settings → Properties change, and starting/tracking a sync from Settings → Data sync (default: `http://localhost:8194/graphql`). Compose overrides it to `http://dagster:8194/graphql`, since Dagster runs as a separate service there. |
| `DAGSTER_UI_URL` | No | Dagster UI address as reached by the **operator's browser** (default: `http://localhost:8194`). Builds the "view technical details" run links on the Data sync tab. Deliberately separate from `DAGSTER_GRAPHQL_URL` — the backend-facing address (a Docker service name) means nothing to a browser, and the incoming request can't be trusted behind a proxy or a remapped port. Set it explicitly when Dagster is served on a hostname or behind a reverse proxy. |
