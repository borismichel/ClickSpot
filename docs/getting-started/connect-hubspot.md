...
"HubSpot API key" app. All scopes are read-only — the pipeline never writes back to
HubSpot.
...
!!! tip "Skipping marketing scopes is fine"
    ...
    corresponding bronze assets just fail to materialize — the CRM pipeline still runs.
...
With the token in place, materialize the assets and the warehouse fills with your data —
