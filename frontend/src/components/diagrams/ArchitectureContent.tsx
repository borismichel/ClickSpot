    ...
    <>
        ...
        >
          ...
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #f5f5f5" }}>
            ...
            <Paragraph type="secondary" style={{ fontSize: 12, margin: "8px 0 0" }}>
              ...
              Disabled objects (Settings → Extraction) are simply not extracted,
              shrinking the count.
            </Paragraph>
          ...
          {/* Anon mirrors */}
          <div style={{ padding: "14px 16px", borderTop: "1px solid #f5f5f5", background: "#fff7e6" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <Text strong>
                <Tag color="volcano" style={{ marginRight: 6 }}>Anon</Tag>
                PII-masked mirrors of silver + gold
              </Text>
              <Text type="secondary" style={{ fontSize: 12 }}>silver_anon + gold_anon</Text>
            </div>
            <Paragraph type="secondary" style={{ fontSize: 12, margin: "8px 0 0" }}>
              After each gold run, an{" "}
              <code>anon_job</code> rebuilds masked copies of every dim/fact/agg
              into <code>silver_anon.*</code> and <code>gold_anon.*</code>. Names,
              emails, phones, addresses, deal names, and free-text fields are
              replaced with fixed-width <code>***</code> tokens; IDs, owners,
              pipelines, and labels survive verbatim. The MCP server queries
              <em> only</em> the anon namespace, so external Claude clients
              physically can't see raw PII.
            </Paragraph>
          </div>
        </div>

        <Divider>Consumers</Divider>
        ...
        <div style={flowSection}>
          <Title level={5} style={{ margin: 0 }}>
            ...
            Five entry points into the warehouse
          </Title>
          <Paragraph type="secondary" style={{ marginTop: 8, marginBottom: 8 }}>
            One pipeline, several ways for humans and clients to use the data.
            All five enforce the same SQL whitelist and dictionary patterns.
          </Paragraph>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>Chat</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Natural language → SQL → viz, inline in the browser
              </div>
            </div>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>Dashboards</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Saved chat results pinned with global filters
              </div>
            </div>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>Data Spaces</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Scoped slices (grain + dimensions) with their own chat &
                dashboards
              </div>
            </div>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>Analytics API</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Associative selection-propagation graph engine
              </div>
            </div>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>MCP Server</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Claude Desktop / other MCP clients query{" "}
                <code>silver_anon</code> / <code>gold_anon</code>
              </div>
            </div>
            <div style={statBox}>
              <Text strong style={{ fontSize: 12 }}>Settings</Text>
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                Onboarding · Extraction · Properties · AI · MCP · Architecture
              </div>
            </div>
          </div>
        </div>
    ...
    { "sql": "SELECT count()...",     "label": "Open Deals" }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 32 }}>
          ...
          <div style={flowSection}>
            ...
            <Paragraph type="secondary" style={{ fontSize: 12, marginBottom: 0, marginTop: 4 }}>
              Core silver columns live as one-line tuples in{" "}
              <code>silver_config.py</code>. Per-portal additions go through{" "}
              Settings → Properties (writes to <code>customer.json</code>) so
              operators don't have to edit Python. DDL, INSERT, the schema
              prompt, and the MCP whitelist all pick up the new columns
              automatically on Dagster reload.
            </Paragraph>
          </div>
